...
void top_level_task(const Task *task,
  ...
  Rect<2> rect_blis(Legion::Point<2>(0, 0),
                    Legion::Point<2>(num_colors - 1, max_size - 1));
  ...
  Rect<2> rect_part(Legion::Point<2>(0, 0),
                    Legion::Point<2>(num_colors - 1, 1));
  IndexSpace is_part = runtime->create_index_space(ctx, rect_part);
  ...
  Legion::Transform<2, 1> tsfm;
  tsfm[0][0] = 1;
  tsfm[1][0] = 0;
  DomainTransform idx_tsfm = tsfm;
  Domain extent(Point<2>(0, 0), Point<2>(0, 1));
  IndexPartition ip_part = runtime->create_partition_by_restriction(
      ctx, is_part, color_is, idx_tsfm, extent, DISJOINT_COMPLETE_KIND);
  ...
  // ---------------------------------------------------------------------
  Rect<2> rect_meta(Legion::Point<2>(0, 0),
                    Legion::Point<2>(1, num_colors - 1));
  IndexSpace is_meta = runtime->create_index_space(ctx, rect_meta);
  ...
  IndexSpace color_meta_is =
      runtime->create_index_space(ctx, color_meta_bounds);

  Domain extent2(Point<2>(0, 0), Point<2>(0, num_colors - 1));
  IndexPartition ip_meta = runtime->create_partition_by_restriction(
      ctx, is_meta, color_meta_is, idx_tsfm, extent2, DISJOINT_COMPLETE_KIND);
  ...
  IndexPartition ip_part_dc = runtime->create_partition_by_image_range(
      ...
      FID_RECT, color_meta_is, DISJOINT_COMPLETE_KIND);
  runtime->attach_name(ip_part_dc, "ip_part_dc");
  LogicalPartition lp_part_dc =
      runtime->get_logical_partition(lr_part, ip_part_dc);
  ...
  // ---------------------------------------
  LogicalRegion lr_blis_used =
      runtime->get_logical_subregion_by_color(ctx, lp_dc, 0);
  IndexSpace is_blis_used = lr_blis_used.get_index_space();

  // ---------------------------------------
  // partition lr_blis_used and lr_part_blis by colors
  // ---------------------------------------
  Domain extent3(Point<2>(0, 0), Point<2>(0, max_size - 1));
  IndexPartition ip_blis_used_dc = runtime->create_partition_by_restriction(
      ctx, is_blis_used, color_is, idx_tsfm, extent3, DISJOINT_COMPLETE_KIND);
  runtime->attach_name(ip_blis_used_dc, "ip_blis_used_dc");
  LogicalPartition lp_blis_used_dc =
      runtime->get_logical_partition(lr_blis_used, ip_blis_used_dc);
  ...
  IndexLauncher check_launcher(CHECK_TASK_ID, color_bounds,
                               TaskArgument(NULL, 0), idx_arg_map);
  check_launcher.add_region_requirement(
      RegionRequirement(lp_blis_used_dc, 0, WRITE_DISCARD, EXCLUSIVE,
                        lr_blis)); // why lr_blis_used doesn't work?
...
void fill_meta_task(const Task *task,
                    const std::vector<PhysicalRegion> &regions, Context ctx,
                    HighLevelRuntime *runtime) {
  assert(regions.size() == 1);
  Legion::Domain dom = runtime->get_index_space_domain(
...
int main(int argc, char **argv) {
...

  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::LOC_PROC));
    registrar.set_leaf();
    Legion::Runtime::preregister_task_variant<check_task>(registrar,
                                                          "check_task");
  } // scope

  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::LOC_PROC));
    registrar.set_leaf();
    Legion::Runtime::preregister_task_variant<fill_meta_task>(registrar,
                                                              "fill_meta_task");
  } // scope
  {
    ...
    registrar.add_constraint(ProcessorConstraint(Processor::LOC_PROC));
    registrar.set_leaf();
    Legion::Runtime::preregister_task_variant<fill_part_task>(registrar,
                                                              "fill_part");
  } // scope
